...
public class CreateUserHandler implements RouteHandler{
	
	private final IAMController controller;
	private final ObjectMapper mapper;
	
	public CreateUserHandler(IAMController controller, ObjectMapper mapper) {
	...
	public APIGatewayProxyResponseEvent handle(APIGatewayProxyRequestEvent request) {
		try {
			...
			IAMUserDTO userDTO = this.controller.createUser(userName);
			String createUserResponse =  this.mapper.writeValueAsString(userDTO);
